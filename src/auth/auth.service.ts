...
export class AuthService {
  ...
  public async googleLogin(data: GoogleLoginDto) {
    ...
    if (user && user.provider !== 'google') {
      return {
        success: false,
        message: `User already registered with ${user.provider}`,
      };
    }

    // Check if the user is already logged in with a valid refresh token
    if (
      user?.refreshToken &&
      ...
      return {
        success: false,
        message: `User already logged in via ${user.provider}`,
      };
    }
  ...
  public async githubLogin(data: GithubLoginDto) {
    ...
    //if user has valid refreshtoken in db
    ...
    //set and get tokens
    const tokens = await this.saveTokenInDb(user.id, user.email, 'github');
